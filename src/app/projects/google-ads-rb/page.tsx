...
export default function GoogleAdsRBPage() {
  ...
  return (
    <ProjectPageLayout
      tag={t.tag}
      title={lang === "zh" ? "Google Ads — 雇主担保推广 (EN)" : "Google Ads — Employer Sponsorship (EN)"}
      subtitle={t.subtitle}
      accent={t.accent}
      impact={t.impact}
    ...
    >
      {/* ── EN Campaign: Keyword Performance ── */}
      <FadeIn delay={0.25}>
        <section>
      ...
      {/* ── Divider ── */}
      <FadeIn delay={0.1}>
        <div className="border-t-2 border-dashed border-blue-100 my-4" />
      </FadeIn>
      ...
      <FadeIn delay={0.15}>
        <section>
          <div className="mb-6">
            ...
            >
              {lang === "zh" ? "项目 5b · 2026年5月19日–25日" : "Project 5b · May 19–25, 2026"}
            </span>
            <h2 className="font-serif text-4xl font-bold mb-1" style={{ color: t.accent }}>
              {lang === "zh" ? "Google Ads — 雇主担保推广 (ZH)" : "Google Ads — Employer Sponsorship (ZH)"}
            </h2>
            <p className="text-gray-500 text-base">
              {lang === "zh"
                ? "职能：IT 总监 — 广告系列创建、策略制定及管理（独立负责）"
      ...
      {/* ── ZH Scale at a Glance ── */}
      <FadeIn delay={0.2}>
        <section>
          <h3 className="font-serif text-2xl font-bold text-gray-900 mb-4">
            {lang === "zh" ? "数据一览" : "Scale at a Glance"}
          </h3>
        ...
        <section>
          ...
          <div className="overflow-x-auto rounded-xl border border-blue-100 shadow-sm">
            <table className="w-full text-sm border-collapse bg-white">
              <thead>
                <tr className="bg-blue-50">
                  <th className="text-left px-4 py-3 font-semibold text-gray-700">{lang === "zh" ? "关键词" : "关键词"}</th>
